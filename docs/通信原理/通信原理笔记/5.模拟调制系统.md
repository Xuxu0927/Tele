...
可以看出,$AM$信号的频谱由载频分量、上边带、下边带三部分组成,其带宽是基带信号带宽$f_H$的两倍
$$
...
$$
其中,$\hat{m}(t)$为$m(t)$的希尔伯特变换,其傅里叶变换为
$$
...
$$
![2ecdecd52f5c4b8ae25ca0e9390415bf](assets/2ecdecd52f5c4b8ae25ca0e9390415bf.png)
...
![63cbdeca529ea036f10319338f685a81](assets/63cbdeca529ea036f10319338f685a81.png)
...
![c98a184d0906d8afb7b93d9e0ab3028f](assets/c98a184d0906d8afb7b93d9e0ab3028f.png)
...
$$
表示最大的相位偏移  $\Delta\omega=K_fA_m$为最大角频偏  $\Delta f=m_f\cdot f_m$为最大频偏
...
![image-20250422181121008](assets/image-20250422181121008.png)
...
与AM信号的频谱
$$
S_{\mathrm{AM}}(\omega)=\pi A[\delta(\omega+\omega_{\mathrm{c}})+\delta(\omega-\omega_{\mathrm{c}})]+\frac{1}{2}[M(\omega+\omega_{\mathrm{c}})+M(\omega-\omega_{\mathrm{c}})]
$$
比较
...
将调制信号积分,然后对载波进行调相,即可产生NBFM信号,再经n次倍频器得到WBFM信号
...
![image-20250422182915988](assets/image-20250422182915988.png)
...
![image-20250422182710332](assets/image-20250422182710332.png)
...
3. 在门限值一下，输出信噪比随输入信噪比的下降而急剧下降，且$m_{f}$越大，输出信噪比的下降越快



## 模拟调制系统的比较

![image-20250422175749311](assets/image-20250422175749311.png)
...
FM占用带宽随调频指数$m_{f}$的增大而增大,其频带利用率最低(利用有效性换取可靠性)

高质量通信$m_{f}$选大些,一般通信$m_{f}$选小些
