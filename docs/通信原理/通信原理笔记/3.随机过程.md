...
或
$$
\begin{cases}P_\xi(f)&=\int_{-\infty}^\infty R(\tau)\mathrm{e}^{-\mathrm{j}\omega\tau}\mathrm{d}\tau\\\\R(\tau)&=\int_{-\infty}^\infty P_\xi(f)\mathrm{e}^{\mathrm{j}\omega\tau}\mathrm{d}f&\end{cases}
$$
简记为
$$
...
$$
Q\left(x\right)=\frac{1}{\sqrt{2\pi}}\int_{x}^{\infty}\mathrm{e}^{-t^{2}/2}\mathrm{d}t\quad x\geqslant0
$$
...
$v_{\mathrm{i}}\left(t\right)$是输入随机过程的一个样本,$v_{o}\left(t\right)$是输出随机过程的一个样本,随机过程$\xi_{\mathrm{i}}\left(t\right)$的样本包括[$v_{\mathrm{i},n}\left(t\right)$ ,$n=1,2,\cdots$]
...
$$
上式表明,若线性系统的输入过程是平稳的,那么输出过程也是平稳的
...
若随机过程$\xi(t)$的谱密度集中在中心频率$f_\mathrm{c}$附近相对窄的频带范围$\Delta f$内，即满足$\Delta f\ll f_{\mathrm{c}}$条件 ,且$f_\mathrm{c}$远离零频率，则称该$\xi(t)$为**窄带随机过程**
...
$$
其中$n(t)$为窄带高斯噪声,其均值为0 方差为$\sigma_{n}^{2}$
$$
...
如果白噪声取值的概率服从高斯分布,则称为**高斯白噪声**,其通常用来作为通信信道中的噪声模型
...
此低通白噪声通常称作带限白噪声,其在$\tau=\frac{k}{2f_{H}}(k=1,2,3)$上得到的随机变量不相关
